...
const GalleryAddPage: NextPage<PageProps> = ({ category, cats }: PageProps) => {
  ...
  const onSubmit = useCallback(
    async (data: any) => {
      if (files.length === 0) {
        toast('error', "You must select at least one video or image to upload.");
        return;
      }
      ...
      <div className="border border-primary">
        <>
          ...
          <div className="mt-8 mb-6 flex justify-center px-4">
            <>
              <form
                className="p-4 shadow-lg rounded-lg border-2 border-secondary max-w-sm"
              ...
              >
                <>
                  ...
                  <input
                    className={`block w-full border border-secondary rounded-lg cursor-pointer focus:outline-none`}
                    id="file_upload"
                    type="file"
                    multiple
                  ...
                  {files.length > 0 && (
                    <>
                      <button
                        onClick={() => setFiles([])}
                        className="mb-3 btn btn-primary btn-sm font-sm text-sm"
                      >
                        Clear Files
                      </button>
